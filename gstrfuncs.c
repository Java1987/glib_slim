...
{ 
	...
	result  = (gchar*)g_malloc((irep > isub ? irep * strlen(source) / isub+ 1:isource) + 1);
	pc1  =  result;
	while(*source  !=  0) 
	{
		pc2  = source; 
...
gboolean g_strstartwith(gchar *string, gchar *sub)
{
	...
	if ((gint)strlen(string) < len) return FALSE;
	for(i = 0; i < len; i++)
	{
		if (string[i] != sub[i]) return FALSE;
	}
	return TRUE;
}
gboolean g_strendwith(gchar *string, gchar *sub)
{
	...
	if (lensrc < len) return FALSE;
	lensrc -= len;
	for(i = 0; i < len; i++)
	{
		if (string[lensrc + i] != sub[i]) return FALSE;
	}
	return TRUE;
}
...
gint g_strlastindexof(gstring fstring, gchar* str)
{
	...
	do 
	{
		...
		if (posnext == -1) return pos == -1 ? -1 : pos - len;
		pos = posnext + len;
	} while (TRUE);
}
gstring g_strsubstring (gstring fstring, gint st, gint len)
{
	...
	if (len <= 0) len = fslen - st;
	if (st + len > fslen) len = fslen - st;
	r = g_malloc(len + 1);
	for(i = 0; i < len; i++)
		r[i] = fstring[st + i];
	r[len] = '\0';
...
{ 
	...
	for(i = 0; i < len; str++, i++) 
	{ 
		hash ^= ((i & 1) == 0) ? ( (hash << 7) ^ ((unsigned int)*str) * (hash >> 3)) : (~((hash << 11) + (((unsigned int)*str) ^ (hash >> 5))));
	}
	return hash;
}
...
gint
g_str_iequal (gconstpointer v, gconstpointer v2)
{
